...
export class EditServiceComponent implements OnInit, OnDestroy {
      ...
      this.subscription = this._customer.getService(this.token.token, this.data.service_id).subscribe(
        response => {
                  ...
                  if (response.status === 'success') {
                    ...
                    this.moreimage = this.services['moreimagen'];
 ...
 loadUserProject(id: number) {
  ...
  onChangeMoreImage(event: any) {

    if (event.checked === true) {
      const tag = 1;
      this._project.moreimagen(this.token.token, this.project_id, this.data.service_id, tag);
      this.snackBar.open('Se ha activado el Modulo de Imágenes en Proyecto.', 'Destacada', {duration: 2000, });
    }

    if (event.checked === false) {
      const tag = 0;
      this._project.moreimagen(this.token.token, this.project_id, this.data.service_id, tag);
      this.snackBar.open('Se ha desactivado el Modulo de Imágenes en Proyecto.', '', {duration: 2000, });
    }
  ...
  filterProjectservicetype() {
    if (this.projectservicetype && this.service_data.type_id) {
      for (let i = 0; i < this.projectservicetype.length; i += 1) {
        const result = this.projectservicetype[i];
  ...
  filterComuna() {
    if (this.comunas && this.service_data.comuna_id) {
      for (let i = 0; i < this.comunas.length; i += 1) {
        const result = this.comunas[i];
