  ...
  private afterRefresch(p: any, i: any): void {
      ...
      if (data.service_value && data.service_value.length > 0 && id > 0) {
          ...
          this._project.addServiceValue(this.token.token, id, service_value)
              .subscribe((resp: any) => {
                      if (!resp) {
                          this.snackBar.open('Error registrando Service Value!!!', '', {
                              duration: 3000,
                          });
                          return;
                      }
                      if (resp.status === 'success') {
                          this.snackBar.open('Solicitud de registro Service Value procesada satisfactoriamente!!!', '', {
                              duration: 1000,
                          });
                      } else {}
                  },
                  error => {
                      this.snackBar.open('Error registrando Service Value!!!', error.error.mensaje, {
                          duration: 3000,
                      });
      ...
      if (data.service_status && data.service_status.length > 0 && id > 0) {
          data.service_status.forEach(res => {
              ...
              this._dataService.addEstatus(this.token.token, service_status, id)
                  .subscribe((resp: any) => {
                          if (!resp) {
                              this.snackBar.open('Error registrando Service Status!!!', '', {
                                  duration: 3000,
                              });
                              return;
                          }
                          if (resp.status === 'success') {
                              this.snackBar.open('Solicitud de registro de Service Status procesada satisfactoriamente!!!', '', {
                                  duration: 3000
                              });
                          } else {}
                      },
                      error => {
                          this.snackBar.open('Error registrando Service Status!!!', error.error.mensaje, {
                              duration: 3000,
                          });
                          console.log( < any > error);
                      }
      ...
      if (data.service_type && data.service_type.length > 0 && id > 0) {
          data.service_type.forEach(res => {
              ...
              this._project.addServiceType(this.token.token, id, service_type)
                  .subscribe((resp: any) => {
                          if (!resp) {
                              this.snackBar.open('Error registrando Service Type!!!', '', {
                                  duration: 3000,
                              });
                              return;
                          }
                          if (resp.status === 'success') {
                              if (res.id && data) {
                                  const lastInsertedSTId = resp.lastInsertedId;

                                  this.filterServiceTypeValue(res.id, data.service_type_value, lastInsertedSTId);
                              }
                              this.snackBar.open('Solicitud de registro de Service Type procesada satisfactoriamente!!!', '', {
                                  duration: 3000
                              });
                              this.forma.reset();
                          } else {}
                      ...
                      error => {
                          this.snackBar.open('Error registrando Service Type!!!', error.error.mensaje, {
                              duration: 3000,
                          });
                          console.log( < any > error);
                      }
      ...
      if (id && data) {
          ...
          this._project.addServiceTypeValue(this.token.token, id, service_type_value)
              .subscribe((resp: any) => {
                      if (!resp) {
                          this.snackBar.open('Error registrando Service Type Value!!!', '', {
                              duration: 3000,
                          });
                          return;
                      }
                      if (resp.status === 'success') {
                          this.snackBar.open('Solicitud de registro de Service Type Value procesada satisfactoriamente!!!', '', {
                              duration: 3000,
                          });
                      } else {}
                  },
                  error => {
                      this.snackBar.open('Error registrando Service Type Value!!!', error.error.mensaje, {
                          duration: 3000,
                      });
                      console.log( < any > error);
                  }
