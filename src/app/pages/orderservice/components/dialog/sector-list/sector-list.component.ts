...
//MODELS
import { Sector } from 'src/app/models/types';
...
export class SectorListComponent implements OnInit, OnDestroy {
  ...
  cargar() {
    this.isLoading = true;
    this.subscription = this.dataService.getSector(this.token.token, this.id)
    .subscribe(
    response => {
              if (!response) {
                this.status = 'error';
              ...
              if(response.status === 'success') {
                this.dataSource = new MatTableDataSource(response.datos.sector);
                this.dataSource.paginator = this.paginator;
                this.dataSource.sort = this.sort;
                this.resultsLength = this.datatype.length;
                this.datatype = response.datos.sector;
  ...
  onSubmit() {
    ...
    this.data = new Sector (0, this.forma.value.descripcion, this.forma.value.order_by, this.forma.value.status, '', '');


    this.dataService.addServiceSector(this.token.token, this.id, this.data)
            .subscribe( (resp: any) => {
              if (!resp) {
  ...
  save(i: number, element: Sector) {
    this.indexitem = i;
    this.editando = false;
    this.isLoadingSave = true;


    this.dataService.updateServiceSector(this.token.token, this.id, element, element.id)
            .subscribe( (resp: any) => {
              if (!resp) {
                this.snackBar.open('Error procesando solicitud!!!', '', {duration: 3000, });
                this.isLoadingSave = false;
  ...
  delete(i: number, element: Sector) {
    this.indexitem = i;
    this.isLoadingDelete = true;

    this.dataService.deleteServiceSector(this.token.token, this.id, element.id)
            .subscribe( (resp: any) => {
              if (!resp) {
                this.snackBar.open('Error procesando solicitud!!!', '', {duration: 3000, });
