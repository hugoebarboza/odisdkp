...
// MODELS
import { Zona } from 'src/app/models/types';
...
export class ZonaListComponent implements OnInit, OnDestroy {
  ...
  cargar() {
    this.isLoading = true;
    this.subscription = this.dataService.getZona(this.token.token, this.id)
    .subscribe(
    response => {
              if (!response) {
                this.status = 'error';
              ...
              if (response.status === 'success') {
                this.dataSource = new MatTableDataSource(response.datos.zona);
                this.dataSource.paginator = this.paginator;
                this.dataSource.sort = this.sort;
                this.datatype = response.datos.zona;
  ...
  onSubmit() {
    ...
    this.data = new Zona (0, this.forma.value.descripcion, this.forma.value.order_by, this.forma.value.status, '', '');

    this.dataService.addServiceZona(this.token.token, this.id, this.data)
            .subscribe( (resp: any) => {
              if (!resp) {
  ...
  save(i: number, element: Zona) {
    this.indexitem = i;
    this.editando = false;
    this.isLoadingSave = true;


    this.dataService.updateServiceZona(this.token.token, this.id, element, element.id)
            .subscribe( (resp: any) => {
              if (!resp) {
                this.snackBar.open('Error procesando solicitud!!!', '', {duration: 3000, });
                this.isLoadingSave = false;
  ...
  delete(i: number, element: Zona) {
    this.indexitem = i;
    this.isLoadingDelete = true;
    this.dataService.deleteServiceZona(this.token.token, this.id, element.id)
            .subscribe( (resp: any) => {
              if (!resp) {
                this.snackBar.open('Error procesando solicitud!!!', '', {duration: 3000, });
