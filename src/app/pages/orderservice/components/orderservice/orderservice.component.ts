...
export class OrderserviceComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  if (this.id > 0 && this.token && this.token.token != null) {
      ...
      const paramp = this.filterProject();
      if (paramp) {
      ...
      const paramservice = this.filterService();
      if (paramservice) {
        this.service = paramservice.service_name;
      }
  ...
  public tabChanged = (_tabChangeEvent: MatTabChangeEvent): void => {
  // console.log('tabChangeEvent => ', tabChangeEvent);
  // console.log('index => ', tabChangeEvent.index);
  }

  loadDataServicio(_servicename: string) {
      // console.log('viene');
